...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [podFolderId, setPodFolderId] = useState<string | null>(null);
  const [podFolderName, setPodFolderName] = useState<string | null>(null);
  ...
  async function loadAll() {
    setLoading(true);
    const [cRes, vRes, rRes, aRes, pRes, fRes] = await Promise.all([
    ...
    if (pRes.ok) {
      ...
      setPodAccess(p.podManagerAccess ?? false);
      setPodFolderId(p.podFolderId ?? null);
      setPodFolderName(p.folderName ?? null);
    }
  ...
  async function togglePodAccess() {
    ...
    try {
      const res = await fetch(`/api/customers/${id}/pod-access`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        toast.success(d.podManagerAccess ? "POD Portal access enabled" : "POD Portal access disabled");
  ...
  async function assignFolder(folderId: string | null) {
    ...
    try {
      const res = await fetch(`/api/customers/${id}/pod-access`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ podFolderId: folderId }),
      });
      if (res.ok) {
        const d = await res.json();
        setPodFolderId(d.podFolderId ?? null);
        setPodFolderName(d.folderName ?? null);
        toast.success(folderId ? `Folder "${d.folderName}" assigned` : "Folder assignment removed");
        setShowFolderPicker(false);
      } else toast.error("Failed to assign folder");
      ...
      <div className="p-6 space-y-6 max-w-4xl">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* POD Portal access toggle */}
            <button
              onClick={togglePodAccess}
              disabled={podAccessLoading}
              title={podAccess ? "Disable customer POD Portal access" : "Enable customer POD Portal access"}
              ...
              POD Portal {podAccess ? "ON" : "OFF"}
            </button>
            {/* POD Folder assignment */}
            <div className="relative">
              <button
                ...
                title="Assign a POD Manager folder for this customer to see in their portal">
                <FolderOpen className="w-3 h-3" />
                {podFolderName ? <span className="max-w-[120px] truncate">{podFolderName}</span> : "Assign Folder"}
              </button>
              {showFolderPicker && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-slate-200 rounded-xl shadow-xl w-64 overflow-hidden">
                  <div className="px-3 py-2 border-b border-slate-100 bg-slate-50">
                    <p className="text-xs font-semibold text-slate-600">Select POD Portal folder</p>
                    <p className="text-xs text-slate-400 mt-0.5">Customer will only see files in this folder</p>
                  </div>
                  <div className="max-h-52 overflow-y-auto py-1">
                    <button
                      onClick={() => assignFolder(null)}
                      disabled={folderPickerLoading}
                      className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-slate-50 ${!podFolderId ? "text-blue-600 font-semibold" : "text-slate-600"}`}>
                      <span className="text-slate-400">—</span> No folder assigned
                    </button>
                    {allFolders.length === 0 && (
                      <p className="px-3 py-2 text-xs text-slate-400">No folders exist yet. Create one in POD Manager first.</p>
                    )}
                    {allFolders.map(f => (
                      <button
                        key={f.id}
                        ...
                        disabled={folderPickerLoading}
                        className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-blue-50 transition-colors ${podFolderId === f.id ? "bg-blue-50 text-blue-700 font-semibold" : "text-slate-700"}`}>
                        <FolderOpen className="w-4 h-4 text-amber-400 shrink-0" />
                        <span className="truncate">{f.name}</span>
                        {podFolderId === f.id && <ShieldCheck className="w-3.5 h-3.5 text-blue-500 ml-auto shrink-0" />}
                      </button>
                    ))}
                  </div>
                  {folderPickerLoading && (
