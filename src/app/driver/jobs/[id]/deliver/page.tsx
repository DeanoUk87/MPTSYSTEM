...
export default function DeliverPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/driver/jobs/${params.id}`)
      ...
      .then(data => { if (!data) { router.back(); return; } setJob(data); setLoading(false); })
      .catch(() => router.back());
  }, [params?.id, router]);
  ...
  async function doSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/driver/jobs/${params.id}/deliver`, { method: "POST", body: fd });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-[#0a0a14] pb-10">
      ...
      <div className="px-4 space-y-4">
        ...
        <div className="bg-[#1c1c2e] rounded-2xl p-4 space-y-4">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { setRecipientAnswer("yes"); if (!signedBy.trim()) setSignedBy(job.deliveryName || ""); }}
